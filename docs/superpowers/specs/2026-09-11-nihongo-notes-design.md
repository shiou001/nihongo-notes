...
## 13. 課程主軸重組（v0.8，2026-09-16）

使用者回饋：「網站缺乏系統性，每樣主題各自分散。」原因：導覽照資料類型（筆記／測驗／五十音／漢字）分，讀、練、複習三步散在三頁；學習地圖只有「開始測驗」按鈕，沒有順序。

決策（使用者選）：用單元當主軸重組；單元順序由我依教學順序排，寫在可編輯的 `data/curriculum.js`。

- **單元模型** `js/curriculum.js`：每級 = 有順序的單元清單。大綱單元（假名／發音／會話／文法／閱讀）來自 `data/curriculum.js`，用 `[頁面標題, 段落關鍵字]` 對應 Notion 段落（讀）與題目出處（練）；單字課（每課 20 字，筆記單字優先）與漢字課（每課 12 字，每字帶 1 個詞裡讀音題）由程式依等級自動切出，穿插在大綱單元之間（每單元後 2 課單字、1 課漢字，剩餘排最後）。N5 共 52 單元。
- **達標**：單元題目 80% 最近一次答對（`seen>0 && streak>0`）即達標；純閱讀單元按「我讀完了」（存成 Progress 的 `u_<key>` 項目，跟著同步）。跨日熟練另計，不當門檻，否則要等好幾天才能前進。
- **目前單元** = 該級第一個未達標的單元。
- **出題**：`buildQuestions` 新增 `opts.ids` 只出指定 id；單元練習題型依單元內容自動決定。
- **導覽**：今日／課程／複習／參考資料。`#/map` 保留為課程頁別名；筆記、五十音、漢字、聲音、同步歸參考資料。
- **頁面**：今日（目前單元＋到期複習）、課程（等級分頁＋單元清單）、單元（讀：筆記段落或單字表或漢字格；練：只出本單元；上一／下一單元）、複習（到期／弱點／各級進度／最近練習）、參考資料。
- **測試**：`sync/curriculum.test.mjs`（解析、穿插順序、達標、範圍鎖定、純閱讀）與 `sync/app.test.mjs`（新頁面渲染、單元測驗流程、我讀完了）。
